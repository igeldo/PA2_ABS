...
def interactive_recommender():
    print("Willkommen zum interaktiven Infektionstherapie-Empfehlungssystem!")

    # Infektionstyp wählen
    print("\nWählen Sie den Infektionstyp:")
    for infection in InfectionType:
        print(f"{infection.value}: {infection.name}")
    infection_type = InfectionType[input("Eingabe (CAP/HAP): ").upper()]

    # Schweregrad wählen
    print("\nWählen Sie den Schweregrad:")
    for severity in Severity:
        print(f"{severity.value}: {severity.name}")
    severity = Severity[input("Eingabe (MILD/MODERATE/SEVERE): ").upper()]

    # Risikofaktoren wählen
    print("\nWählen Sie die Risikofaktoren:")
    for risk in RiskFactors:
        print(f"{risk.value}: {risk.name}")
    risk_factors = RiskFactors[input("Eingabe (NONE/COMORBIDITY/PSEUDOMONAS): ").upper()]

    # Penicillinallergie wählen
    print("\nWählen Sie den Penicillinallergiegrad:")
    for allergy in PenicillinAllergy:
        print(f"{allergy.value}: {allergy.name}")
    penicillin_allergy = PenicillinAllergy[input("Eingabe (NONE/MILD/SEVERE): ").upper()]

    # Optionaler Erreger
    print("\nMöchten Sie einen spezifischen Erreger angeben? (ja/nein)")
    if input().lower() == 'ja':
        print("\nWählen Sie den Erreger:")
        for pathogen in Pathogen:
            print(f"{pathogen.value}: {pathogen.name}")
        pathogen = Pathogen[input("Eingabe: ").upper()]
    else:
        pathogen = None

    # Empfehlung generieren
    recommendation = recommend_therapy(infection_type, severity, risk_factors, penicillin_allergy, pathogen)

    print("\nEmpfohlene Therapie:")
    print(recommendation)


# Führen Sie die interaktive Funktion aus
